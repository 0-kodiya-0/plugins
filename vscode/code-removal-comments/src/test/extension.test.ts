...
describe('Code Removal Comments Extension', () => {
  ...
  console.log("normal line");
      ...
      assert.ok(
        wrappedLine?.includes('const debug = true'),
        'Should contain the selected code inline'
      );
    });
  });
  ...
  describe('Indentation Preservation', () => {
    ...
    if (true) {
      ...
      assert.ok(
        startCommentLine?.startsWith('        '),
        'Start comment should preserve indentation'
      );
  ...
  /* BUILD_REMOVE_END */
      ...
      const text = document.getText();
      const lines = text.split('\n');
      const convertedLine = lines.find(
        (line) =>
