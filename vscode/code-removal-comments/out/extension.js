...
function wrapWithComments(editor, startMarker, endMarker, useMultiLineStyle = true, inline = false) {
...
function removeExistingMarkers(editor) {
    const config = getConfig();
    const selection = editor.selection;
    const selectedText = editor.document.getText(selection);
    if (!selectedText) {
...
function convertToInline(editor) {
    const selection = editor.selection;
    const selectedText = editor.document.getText(selection);
    if (!selectedText) {
        vscode.window.showWarningMessage('No text selected');
...
function activate(context) {
    ...
    const configure = vscode.commands.registerCommand('codeRemoval.configure', async () => {
        ...
        switch (choice) {
            ...
            case 'Reset to Defaults':
                const confirmReset = await vscode.window.showWarningMessage('This will reset all settings to defaults. Continue?', 'Yes', 'No');
                if (confirmReset === 'Yes') {
    ...
    context.subscriptions.push(wrapMultiLine, wrapSingleLine, wrapInlineMultiLine, wrapProductionOnly, wrapDevelopmentOnly, wrapTestOnly, wrapDebugBlock, markLine, removeMarkers, convertInline, configure, quickWrap);
